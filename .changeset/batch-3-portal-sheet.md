...
- **Sheet** – a bottom sheet on Portal: scrim + background dim, focus trap with restore, body scroll lock, close on `Escape` / scrim tap / header button / downward drag past a threshold or flick. `dismissable={false}` removes every implicit close. Reduced motion swaps the slide for a fade. Fixed height via `--tgui--sheet--height`; pass a `container` to scope it to a box instead of the viewport.
