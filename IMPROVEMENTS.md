...
- **Real package.** ESM `exports` map, bundled `.d.ts`, `sideEffects` limited to the CSS, React externalised as a peer dependency, `publint` + `arethetypeswrong` clean. The original's packaging broke repeatedly (#3, #22, #107, #108, PR #6). One stylesheet at `telegram-ui/styles.css`.
